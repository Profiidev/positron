use axum::body::Body;
use centaurus::{error::Result, storage::StoragePath};
...
impl<'b> ApodFolder<'b> {
  ...
  pub async fn upload<R: AsyncRead + Unpin + Send>(&self, path: &str, image: &mut R) -> Result<()> {
    self
      .storage
      .save_file(image, self.base.clone().join(path))
      .await
  }
  ...
  pub async fn download(&self, path: &str) -> Result<Body> {
    self
      .storage
      .get_file(&self.base.clone().join(path), None)
      .await
  }

  pub async fn exists(&self, path: &str) -> Result<bool> {
    self.storage.exists(&self.base.clone().join(path)).await
  }
}
